$(document).ready(function() {
    $(".zxf_pagediv").createPage({
        pageNum: parseInt($("#pageTotal").val()),
        current: parseInt($("#pageCur").val()),

        backfun : function (e) {
            var index = parseInt(e.current);
            var currentPage = parseInt($("#pageCur").val());
            var pageTotal = parseInt($("#pageTotal").val());

            if (index == 1) {//首页
                document.getElementById("indexLink").click();
            } else if (index == pageTotal) {//尾页
                document.getElementById("lasterLink").click();
            } else if (currentPage == index + 1) { //上一页
                document.getElementById("topLink").click();
            } else if (currentPage == index - 1) { //下一页
    ...
    $('.contentTitle').children().click(function() {
        let index = $(this).index()
        $('.newsContent1').children().eq(index).show().siblings().hide()
        $(this).addClass('defaultS').siblings().removeClass('defaultS')
    })
